...
def _get_expected_file(what):
    try:
        ...
        print('then check the --help (and probably the code too, because i\'m note going to write a proper doc for this)')
        sys.exit(1)
...
def app_run_check():
    ...
    for name, w_info in watchers.items():
        ...
        report[name] = []
        path, url, known_commits = w_info['path'], w_info['URL'], set(w_info['known_commits'])
        path = os.path.expanduser(path)
        dir_path = _path_to_first_parent_dir(path)
        cmd = ['git', '-C', dir_path, 'log', '--pretty=format:%h!|!%s', path]
        git_log = subprocess.run(cmd, capture_output=True).stdout.decode("utf-8").strip()
