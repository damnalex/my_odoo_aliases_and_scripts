...
ssho() {
    ...
    if [[ $1 = "test.upgrade" ]] || [[ $1 = "upgrade" ]]; then
        echo '\n\n\n\n\n `odoo-upgrade-get-request <download_token>` to get the dump\n\n\n\n\n\n\n'
...
test-dump() {
    ...
    python3 <(curl -s 'https://raw.githubusercontent.com/mao-odoo/all_standard_odoo_apps_per_version/main/is_my_module_standard.py') $db_version -m $(psql -tAqX -d "oes_$db_name" -c "SELECT name from ir_module_module where state not in ('uninstalled', 'uninstallable');") | grep -A100 "Third-party Modules"
    echo "------------"
